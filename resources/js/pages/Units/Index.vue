<template>
    <div class="p-8 max-w-7xl mx-auto">
        ...
        <div v-else-if="error" class="bg-white rounded-lg shadow-sm p-12 text-center">
            ...
            <button @click="fetchUnits" class="mt-4 px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors">
                Retry
            </button>
        </div>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        <tr v-if="units.length === 0">
                            <td colspan="6" class="px-4 py-12 text-center">
                                ...
                                <p class="text-gray-600">No units found</p>
                            </td>
                        </tr>
                        <tr v-for="unit in units" :key="unit.id" class="hover:bg-gray-50">
                            <td class="px-4 py-4 text-sm font-semibold text-gray-900">{{ unit.name }}</td>
                            <td class="px-4 py-4">
                                <span class="inline-block px-3 py-1 text-xs font-medium bg-blue-100 text-blue-800 rounded-full">
                                    {{ unit.short_name }}
                                </span>
                            </td>
                            <td class="px-4 py-4 text-sm text-gray-600">{{ unit.description || 'N/A' }}</td>
                            <td class="px-4 py-4 text-sm text-gray-600">{{ unit.products_count || 0 }}</td>
                            <td class="px-4 py-4">
                                <button
                                ...
                                >
                                    <span v-if="statusLoading[unit.id]" class="inline-block w-4 h-4 border-2 border-current border-t-transparent rounded-full animate-spin"></span>
                                    <span v-else>{{ unit.is_active ? 'Active' : 'Inactive' }}</span>
                                </button>
                            </td>
                            <td class="px-4 py-4">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    >
                                        <span v-if="deleteLoading[unit.id]" class="inline-block w-4 h-4 border-2 border-current border-t-transparent rounded-full animate-spin"></span>
        ...
        <div v-if="showDeleteModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" @click.self="showDeleteModal = false">
            <div class="bg-white rounded-lg w-full max-w-md shadow-xl">
                ...
                <div class="p-6">
                    ...
                    <p v-if="unitToDelete?.products_count > 0" class="text-red-600 flex items-center gap-2">
                        <i class="fas fa-exclamation-triangle"></i>
                        <span>This unit has {{ unitToDelete.products_count }} associated products.</span>
                    </p>
                </div>
...
export default {
    ...
    methods: {
        async fetchUnits() {
            ...
            try {
                ...
                if (response.data.success) {
                    ...
                    this.error = response.data.message || 'Failed to load units';
                }
            } catch (err) {
                console.error('Error fetching units:', err);
                this.error = err.response?.data?.message || 'An error occurred while loading units';
            } finally {
                this.loading = false;
            }
        },
        ...
        async toggleStatus(unit) {
            this.statusLoading[unit.id] = true;

            try {
                const response = await axios.patch(`/api/units/${unit.id}/toggle-status`);
                ...
                delete this.statusLoading[unit.id];
            }
        },
        confirmDelete(unit) {
        ...
        async deleteUnit() {
            ...
            try {
                const response = await axios.delete(`/api/units/${this.unitToDelete.id}`);

                if (response.data.success) {
                    this.toast.success(response.data.message || 'Unit deleted successfully');
                    this.showDeleteModal = false;
                    this.unitToDelete = null;
                    this.fetchUnits();
                } else {
                    this.toast.error(response.data.message || 'Failed to delete unit');
                }
            } catch (err) {
                console.error('Error deleting unit:', err);
                this.toast.error(err.response?.data?.message || 'Failed to delete unit');
            } finally {
