    ...
    <div class="p-8 max-w-7xl mx-auto">
        ...
        <div class="flex justify-between items-center mb-8">
            <div>
                ...
                <h1 class="text-3xl font-bold text-gray-800">Trashed Stock In Records</h1>
                <p class="text-gray-600 mt-1">View and manage deleted stock in transactions</p>
            </div>
            <router-link
                to="/stock-ins"
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
            ...
            <div v-else-if="stockIns.length === 0" class="p-12 text-center">
                <i class="fas fa-trash-restore text-gray-400 text-5xl mb-4"></i>
                <p class="text-gray-600 text-lg">No trashed records found</p>
                <router-link to="/stock-ins" class="text-blue-600 hover:text-blue-700 mt-2 inline-block">
                    Go to Stock In
                </router-link>
            </div>
            ...
            <div v-else class="overflow-x-auto">
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        <tr v-for="stockIn in stockIns" :key="stockIn.id" class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 text-sm text-gray-900">
                                {{ stockIn.product_count }} item{{ stockIn.product_count !== 1 ? 's' : '' }}
                            </td>
                            <td class="px-6 py-4 text-sm">
                                <span :class="getStatusClass(stockIn.status)" class="px-3 py-1 text-xs font-medium rounded-full">
                            ...
                            <td class="px-6 py-4 text-sm text-gray-500">
                                {{ formatDateTime(stockIn.deleted_at) }}
                            </td>
                            <td class="px-6 py-4 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <button
                                        @click="confirmRestore(stockIn)"
                                    ...
                                    <button
                                        @click="confirmPermanentDelete(stockIn)"
...
export default {
    ...
    mounted() {
        this.fetchTrashedStockIns();
    },
    methods: {
        async fetchTrashedStockIns(page = 1) {
            this.loading = true;
            try {
                const params = {
                    page: page,
                ...
                const response = await axios.get('/api/stock-ins/trashed', { params });

                if (response.data.success) {
                    this.stockIns = response.data.data.data;
                    this.pagination = {
                ...
                console.error('Error fetching trashed stock ins:', err);
        ...
        debouncedSearch() {
            ...
            this.searchTimeout = setTimeout(() => {
                this.fetchTrashedStockIns(1);
            }, 500);
        },

        changePage(page) {
            if (page !== this.pagination.current_page) {
                this.fetchTrashedStockIns(page);
            }
        },

        confirmRestore(stockIn) {
            if (confirm(`Are you sure you want to restore "${stockIn.reference_number}"?`)) {
                this.restoreStockIn(stockIn.id);
            }
        },

        async restoreStockIn(id) {
            try {
                const response = await axios.post(`/api/stock-ins/${id}/restore`);
                if (response.data.success) {
                    this.toast.success('Stock in restored successfully');
                    this.fetchTrashedStockIns(this.pagination.current_page);
                }
            } catch (err) {
                console.error('Error restoring stock in:', err);
                this.toast.error(err.response?.data?.message || 'Failed to restore stock in');
            }
        },

        confirmPermanentDelete(stockIn) {
            if (confirm(`Are you sure you want to permanently delete "${stockIn.reference_number}"? This action cannot be undone!`)) {
                this.permanentlyDelete(stockIn.id);
            }
        },

        async permanentlyDelete(id) {
            try {
                const response = await axios.delete(`/api/stock-ins/${id}/force-delete`);
                if (response.data.success) {
                    this.toast.success('Stock in permanently deleted');
                    this.fetchTrashedStockIns(this.pagination.current_page);
                }
            } catch (err) {
